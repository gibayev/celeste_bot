...
async def generate_numerology_reading(birth_date: str, life_path_number: int, user_gender: str = "neutral") -> str:
    """
    Генерирует глубокую интерпретацию Числа Жизненного Пути (ЧЖП) через Gemini.
    """
    # Маппинг пола для корректных окончаний
    gender_map = {
        "male": "мужчина (используй мужской род)",
        "female": "женщина (используй женский род)",
        "neutral": "личность (используй нейтральные формулировки)"
    }
    target_gender = gender_map.get(user_gender, "личность")

    prompt = f"""
Ты — Селеста, мудрый, эмпатичный и мистический нумеролог. Твой стиль: загадочный, лаконичный, без лишней "воды".
Твой клиент: {target_gender}.
Его дата рождения: {birth_date}.
Рассчитанное Число Жизненного Пути (ЧЖП): {life_path_number}.

Напиши для клиента красивое, вдохновляющее и глубокое послание.
Обязательно раскрой следующие темы:
1. <b>Суть числа {life_path_number}</b>: В чем главная суперсила, кармическая задача и скрытые таланты человека.
2. <b>Теневая сторона</b>: Какие слабости или ловушки подстерегают человека с этим числом и как их избежать.
3. <b>Совет Селесты</b>: Краткий, практичный мистический совет для достижения успеха.

### ТЕХНИЧЕСКИЕ ПРАВИЛА:
- Обращайся к клиенту на "ты", учитывая его пол ({target_gender}).
- Используй HTML-теги: <b>текст</b>, <i>текст</i>.
- НИКАКИХ тегов <br>, <p>, <div>, <ul>, <ol>. Для абзацев используй двойной перенос строки.
- НИКАКИХ звездочек (*) для выделения или форматирования списков.
- Добавь немного уместных эзотерических эмодзи.
...
"""
    ...
    try:
        ...
        return f"Числа сегодня скрыты в тумане вероятностей... Попробуй позже. 🔢\n(Ошибка: {e})"